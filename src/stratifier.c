...
/* Parasite payout mode (finder_percent > 0): the block finder receives a flat
 * percentage of the reward via a personalized per-user coinbase, and is
 * excluded from the proportional split of the remainder. Reuses the SOLO-mode
 * per-user workbase machinery. Meaningless in solo and proxy modes. */
#define PARA_MODE(ckp) ((ckp)->finder_percent > 0.0 && !(ckp)->solo && !(ckp)->proxy)

/* One planned proportional payout slot in parasite mode, snapshot at workbase
 * generation time. Stored as an array in wb->para.slots. */
struct para_slot {
    struct user_instance *user;
    double herp;
};
...
/* Parasite payout mode: plan the payouts for this workbase instead of adding
 * them to cb1 (see add_user_generation). No miner payout outputs are added to
 * cb1 at all: every one of them lives in the per-user coinb2 (see
 * find_or_generate_userwb__) so that each miner's personalized job pays
 * themselves the flat finder bonus and splits the remainder proportionally
 * among everyone else. This function computes the global payout plan, stores
 * it in wb->para and sets wb->payout for record keeping.
 * avail is the reward remaining after the pool fee. */
static void plan_para_payouts(const pool_t *ckp, sdata_t *sdata, workbase_t *wb, const uint64_t g64,
                              const uint64_t avail, const uint64_t f64, const uint64_t pf64, const uint64_t df64)
{
    json_t * const payout = json_object(), * const payout_entries = json_object(),
           * postponed_entries = NULL;
    generation_t paygens[PAYOUT_REWARDS + 1];
    struct para_slot *slots;
    double total_herp, slot_herp = 0.;
    uint64_t finder, reward;
    int n = 0;

    /* The finder bonus is a percentage of the full block reward (subsidy +
     * txn fees), capped at what is left after the pool fee */
    finder = (uint64_t)floor(g64 * (ckp->finder_percent / 100.0));
    if (finder > avail)
        finder = avail;
    reward = avail - finder;

    /* Copy the current values stored in pool stats generated by
     * calc_user_paygens, exactly as add_user_generation does. */
    mutex_lock(&sdata->stats_lock);
    memcpy(paygens, sdata->stats.paygens, sizeof(paygens));
    postponed_entries = json_copy(sdata->stats.postponed);
    ...
    slots = ckzalloc(sizeof(*slots) * (PAYOUT_REWARDS + 1));
    for (int i = 0; i < PAYOUT_REWARDS && total_herp > 0.; ++i) {
        generation_t * const gen = &paygens[i];
        user_instance_t * const user = gen->user;
        uint64_t planned;

        if (!user)
            break;
        /* Dust test with the full denominator: per-user amounts (finder
         * excluded from the denominator) can only be larger, so a slot that
         * passes here is above dust in every user's job. */
        planned = (uint64_t)floor(reward * (gen->herp / total_herp));
        if (planned < PAYOUT_DUST) {
            json_set_double(postponed_entries, user->username, gen->herp);
            continue;
        }
        slots[n].user = user;
        slots[n].herp = gen->herp;
        slot_herp += gen->herp;
        n++;
        /* Informational: the actual on-chain amount depends on who finds the
         * block, since the finder is excluded from the split */
        json_set_double(payout_entries, user->username, planned / (double)SATOSHIS);
    }

    wb->para.slots = slots;
    wb->para.n_slots = n;
    wb->para.total_herp = slot_herp;
    wb->para.reward = reward;
    wb->para.finder = finder;
    /* Every per-user coinb2 has exactly n_slots + 1 outputs (see
     * find_or_generate_userwb__): 8 byte amount + 1 byte pushlen + script
     * each, plus 4 nlocktime bytes, on top of the shared coinb2 prefix. */
    wb->para.max_user_cb2 = (n + 1) * (8 + 1 + GENERATOR_MAX_CSCRIPT_LEN) + 4;
    ...
    json_set_double(payout, "finder_percent", ckp->finder_percent);
    json_set_double(payout, "finder_reward", finder / (double)SATOSHIS);
    json_object_set_new_nocheck(payout, "payouts", payout_entries);
    json_set_double(payout, "herp", total_herp);
    json_object_set_new_nocheck(payout, "postponed", postponed_entries);
    json_set_int64(payout, "time", (int64_t)time(NULL));
    ...
    LOGINFO("Parasite payout plan: finder %1.8f + %1.8f split over %d payees",
            finder / (double)SATOSHIS, reward / (double)SATOSHIS, n);
}
...
#define SET_POOL_AMT(amt) \
    do { \
        ...
        if (PARA_MODE(ckp)) {
            /* Parasite payout mode: all miner payouts live in the per-user
             * coinb2, not cb1. Plan them now and return like SOLO does. */
            plan_para_payouts(ckp, sdata, wb, g64, g64 - f64, f64, pf64, df64);
            return 0u;
        }
...
#define compact_size_reserved 3
    ...
    if (unlikely(wb->coinb1len + wb->enonce1varlen + wb->enonce2varlen + wb->coinb2len
                 + wb->para.max_user_cb2 /* 0 unless parasite mode */ > MAX_COINBASE_TX_LEN)) {
...
/* Append one full txout (8 byte LE amount + 1 byte pushlen + scriptPubKey) at
 * buf[pos], returning the new position. Parasite mode only. */
static int append_cb2_output_(uchar *buf, int pos, const uint64_t amount, const uint8_t *script, const int scriptlen)
{
    const uint64_t le_amt = htole64(amount);

    memcpy(buf + pos, &le_amt, sizeof(le_amt));
    pos += sizeof(le_amt);
    buf[pos++] = (uint8_t)scriptlen; /* Assumption: compactsize is 1 byte here (scripts are always < 76 bytes) */
    memcpy(buf + pos, script, scriptlen);
    return pos + scriptlen;
}
...
static struct userwb * find_or_generate_userwb__(sdata_t *sdata, const workbase_t *wb, user_instance_t *user)
{
    ...
    userwb->id = id;
    if (PARA_MODE(sdata->ckp)) {
        /* Parasite mode: build this user's entire payout section. The user
         * (as the would-be block finder) receives the flat finder bonus and
         * is excluded from the proportional split of the remainder, whose
         * denominator excludes their score, Parasite-style. Every user's
         * coinb2 has exactly n_slots + 1 outputs to match the output count
         * declared in cb1: users with a slot of their own get their finder
         * bonus split across two outputs to keep the count uniform. */
        const struct para_slot * const slots = wb->para.slots;
        const int n_slots = wb->para.n_slots;
        double denom = wb->para.total_herp;
        uint64_t allocated = 0, finder_amt;
        int outs = 0, pos;
        ...
        userwb->coinb2bin = ckalloc(wb->coinb2len + wb->para.max_user_cb2);
        memcpy(userwb->coinb2bin, wb->coinb2bin, wb->coinb2len);
        pos = wb->coinb2len;

        if (denom > 0.) {
            for (int i = 0; i < n_slots; ++i) {
                user_instance_t * const payee = slots[i].user;
                uint64_t amt;

                if (payee == user)
                    continue;
                amt = (uint64_t)floor(wb->para.reward * (slots[i].herp / denom));
                pos = append_cb2_output_(userwb->coinb2bin, pos, amt, payee->scriptbin, payee->scriptlen);
                allocated += amt;
                outs++;
            }
        }

        /* The finder bonus, plus any unallocated remainder (flooring dust,
         * postponed sub-dust slices, or the whole remainder if this user is
         * the only participant), goes to this user */
        finder_amt = wb->para.finder + (wb->para.reward - allocated);
        if (n_slots - outs > 0) {
            /* This user has a slot of their own: emit two finder outputs so
             * every job declares the same output count. A half below the
             * dust limit is still consensus-valid in a coinbase. */
            const uint64_t half = finder_amt / 2;

            pos = append_cb2_output_(userwb->coinb2bin, pos, half, user->scriptbin, user->scriptlen);
            pos = append_cb2_output_(userwb->coinb2bin, pos, finder_amt - half, user->scriptbin, user->scriptlen);
        } else {
            pos = append_cb2_output_(userwb->coinb2bin, pos, finder_amt, user->scriptbin, user->scriptlen);
        }

        memcpy(userwb->coinb2bin + pos, wb->solo.coinb3bin, wb->solo.coinb3len);
        userwb->coinb2len = pos + wb->solo.coinb3len;
...
retry:
    ...
    if (ckp->solo || PARA_MODE(ckp))
        // SOLO/parasite mode: generate per-user workbases and broadcast
...
{
    if (client && (client->ckp->solo || PARA_MODE(client->ckp))) {
                         ...
                         uchar *hash, uchar * const swap, int *cblen)
{
    ...
    *cblen += wb->enonce2varlen;
    if (client && client->sdata && (client->ckp->solo || PARA_MODE(client->ckp))) {
        // SOLO/parasite mode only, write user-specific coinb2
        const uchar *cb2bin;
        int cb2len;
                                                  ...
                                                  int server)
{
    ...
    if (ckp->startdiffs && ckp->startdiffs[server] > 0)
        client->diff = client->old_diff = ckp->startdiffs[server];
    else
...
/* Reset all share accounting on a block solve when finder_percent payouts are
 * active: scores accumulate only between blocks found by this pool. Dust
 * carry-forward (user->accumulated/postponed) deliberately survives the reset
 * so sub-dust miners eventually get paid. Rolling totals reset to 0.1, the
 * same divide-by-zero sentinel used in read_userstats(). */
static void reset_shareaccounting(sdata_t *sdata)
{
    user_instance_t *user, *tmpuser;

    mutex_lock(&sdata->uastats_lock);
    sdata->stats.unaccounted_herp = 0;
    sdata->stats.unaccounted_lns = 0;
    ...
    HASH_ITER(hh, sdata->user_instances, user, tmpuser) {
        ...
        mutex_lock(&user->stats_lock);
        user->herp = user->lns = user->ua_herp = user->ua_lns = 0;
        DL_FOREACH(user->worker_instances, worker) {
            worker->herp = worker->lns = worker->ua_herp = worker->ua_lns = 0;
        }
        mutex_unlock(&user->stats_lock);
    }
    ck_wunlock(&sdata->instance_lock);
...
static void block_solve(pool_t *ckp, json_t *val)
{
    ...
    if (ckp->finder_percent > 0.0) {
        /* Parasite payouts: reset share accounting for the new round and
         * refresh the paygen snapshot before the base update below, so the
         * next workbase doesn't carry the previous round's payout plan */
        reset_shareaccounting(sdata);
...
out:
    if (ret && (ckp->solo || PARA_MODE(ckp)) && !client->remote) {
        sdata_t *sdata = ckp->sdata;
        workbase_t *wb;
                       ...
                       const double diff, const bool valid, const bool submit)
{
    ...
    if (valid) {
        if (ckp->finder_percent > 0.0)
            /* Parasite-style payouts are strictly proportional to accepted
             * share difficulty, with no variance smoothing */
            herp = diff;
        else
                              ...
                              const char *nonce, uchar *hash, const bool stale, bool *solved)
{
    ...
    coinbase = ckalloc(wb->coinb1len + wb->enonce1constlen + wb->enonce1varlen + wb->enonce2varlen + wb->coinb2len
                       ...
                       + wb->para.max_user_cb2 /* per-user payout section (parasite mode only, else 0) */
                       + wb->solo.coinb3len);
...
static void update_client(const stratum_instance_t *client, const int64_t client_id)
{
    ...
    if (!client->ckp->solo && !PARA_MODE(client->ckp))
...
static void *statsupdate(void *arg)
{
    ...
    while (42) {
         ...
         * With finder_percent payouts there is no window: scores accumulate
         * from one found block to the next and reset on block solve. */
        if (ckp->finder_percent == 0.0 && stats->rolling_herp + herp > stats->herp_window) {
        ...
        if (ckp->finder_percent == 0.0 && stats->rolling_lns + lns > stats->herp_window) {
        ...
        /* Guard division: network_diff is stored as uint64_t and truncates to
         * zero on regtest where difficulty < 1 (SIGFPE with FP traps enabled) */
        percent = stats_copy->network_diff ?
                (double)stats_copy->accounted_diff_shares * 100.0 / (double)stats_copy->network_diff : 0.0;
